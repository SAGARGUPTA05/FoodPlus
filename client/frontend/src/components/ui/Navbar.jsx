...
function Navbar() {
  ...
  return (
    <div className={`w-full ${theme ==="light" ? 'bg-white' : 'bg-[#121212]'}  fixed top-0 left-0 z-[300] `}>
      <div className="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 md:px-20 h-16">
        {/* Logo */}
        <Link to="/" className="text-black no-underline">
          <h1 className={`font-bold md:font-extrabold text-2xl ${theme=="light" ?'text-black' :'text-[#E0E0E0]'}`}>
            FoOdplus
          </h1>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-x-4">
          ...
          {/* Profile Picture */}
          {user && (
            <div className="w-12 h-12 flex justify-center items-center  text-white rounded-full font-semibold border-4">
          ...
          <Link to="/cart" className="relative cursor-pointer">
            ...
            {cart.length > 0 && (
              <div className={`absolute -top-2 -right-2 text-xs rounded-full w-4 h-4 bg-red-500 ${theme=="light" ?'text-black' :'text-[#E0E0E0]'} flex items-center justify-center`}>
                {cart.length}
              </div>
            )}
          </Link>
